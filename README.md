...
朋友之间一键共享屏幕。无需云服务器，浏览器直连。

## 怎么用

**主叫方（你）**：
1. 双击打开 `screenshare.html`
2. 点 "创建房间" → 立刻得到一个 12 位房间号 + 分享链接
3. 把链接发给朋友（微信、QQ 邮件都行）
...
| 谁 | 看到什么 |
|---|---|
| 浏览器之间 | 屏幕画面 + 系统音频，端到端加密（DTLS-SRTP） |
| PeerJS 公共 broker | 你的 IP、房间号、"用户 A 想连到房间 xyz" — **看不到也解不开**屏幕数据 |
...
- 如果双方都是"严格对称 NAT"（极少见），可能连不上 — 解法：一方用 VPN/Tailscale/ZeroTier 创造虚拟局域网
